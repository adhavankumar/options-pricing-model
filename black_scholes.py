"""Black-Scholes closed-form pricing, analytical Greeks, and Newton-Raphson implied volatility solver."""
import numpy as np
...
def implied_vol(S, K, T, r, market_price, option_type):
    ...
    for i in range(100): # maximum 100 iterations, Newton Raphson
        if(option_type == "call"):
        ...
        if(abs(error) < tol_err): # check if error is within tolerance, if it is we have found sigma
            return sigma
        if(abs(Vega) < tol_zero): # check is vega is close to zero, if it is stop Newton Raphson
            break
